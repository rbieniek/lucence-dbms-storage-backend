...
@SpringBootTest(webEnvironment = WebEnvironment.NONE, classes = JdbcIndexOutputIntegrationTest.TestConfig.class, properties = "spring.datasource.url=jdbc:h2:mem:JIOIT;DB_CLOSE_ON_EXIT=FALSE")
			...
			try {
				...
				if (rs.next()) {
					return Optional.of(LuceneFileExtent.builder().id(UUID.fromString(rs.getString(1)))
							...
							.extentLength(rs.getInt(4)).extentData(Base64.getDecoder().decode(rs.getString(5))).build());
				} else {
